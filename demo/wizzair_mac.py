...
import never_primp
import uuid
from demo.mac_call_edge_sandbox import call_javascript
...
# session = never_primp.Client(impersonate='chrome_148', proxy=proxyStr, impersonate_os='macos')
...
print(tl_req.requests[0].body)
print(f'[请求数] {len(tl_req.requests)}', )
# 取出/tl的header以及body
headers = dict(tl_req.requests[-1].headers)
data = tl_req.requests[-1].body
print(f'[tl body length] {len(data)}')
url = tl_req.requests[-1].url
# print(headers)
