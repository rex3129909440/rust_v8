#[derive(Default)]
pub(crate) struct OnBeforeInstallPromptStore {
    handler: Option<v8::Global<v8::Value>>,
}

pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(OnBeforeInstallPromptStore::default());
}
...
pub(crate) fn dispatch(
    ...
    if event_type != "beforeinstallprompt" {
        return;
    }
    let global = scope.get_current_context().global(scope);
...
fn set_onbeforeinstallprompt(
    scope: &mut v8::PinScope<'_, '_>,
    arguments: v8::FunctionCallbackArguments<'_>,
    _: v8::ReturnValue<'_>,
) {
    let handler = super::window_event_handler_support::handler_value(scope, arguments.get(0));
    if let Some(store) = scope.get_slot_mut::<OnBeforeInstallPromptStore>() {
