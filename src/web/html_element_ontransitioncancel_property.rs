...
pub(crate) fn define(
    ...
    crate::webidl::define_accessor(scope, prototype, "ontransitioncancel", get_value, set_value)
}
...
fn get_value(
    ...
    if let Some(handler) = record.handlers.get("ontransitioncancel") {
...
fn set_value(
    ...
    if let Some(record) = scope.get_slot_mut::<HtmlElementStore>().and_then(|store| {
        ...
        if let Some(handler) = handler {
            record
                .handlers
                .insert("ontransitioncancel".to_owned(), handler);
        } else {
            record.handlers.remove("ontransitioncancel");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
