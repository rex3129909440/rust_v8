#[derive(Default)]
pub(crate) struct OnWebkitAnimationIterationStore {
    handler: Option<v8::Global<v8::Value>>,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(OnWebkitAnimationIterationStore::default());
}
...
pub(crate) fn dispatch(
    ...
    if event_type != "webkitanimationiteration"
        || !super::window_event_handler_support::is_window(scope, target)
    {
        return;
    }
    let handler = scope
        .get_slot::<OnWebkitAnimationIterationStore>()
...
fn set_handler(
    ...
    if let Some(store) = scope.get_slot_mut::<OnWebkitAnimationIterationStore>() {
