...
#[derive(Default)]
pub(crate) struct RtcEncodedAudioFrameStore {
    constructor: crate::webidl::RealmConstructor,
    records: HashMap<i32, AudioFrameRecord>,
}
...
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(RtcEncodedAudioFrameStore::default());
}

pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let constructor = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "RTCEncodedAudioFrame", constructor.into())
}
...
fn ensure_constructor<'s>(
    ...
    let existing = scope
        .get_slot::<RtcEncodedAudioFrameStore>()
    ...
    scope
        .get_slot_mut::<RtcEncodedAudioFrameStore>()
        .ok_or_else(|| "RTCEncodedAudioFrame state was not prepared".to_owned())?
        .constructor
...
fn construct(
    ...
    if !arguments.is_construct_call() || arguments.length() < 1 {
        crate::webidl::throw_type_error(
            scope,
            "Failed to construct 'RTCEncodedAudioFrame': 1 argument required",
        );
        return;
    }
    let Ok(source) = v8::Local::<v8::Object>::try_from(arguments.get(0)) else {
        crate::webidl::throw_type_error(
            scope,
            "Failed to construct 'RTCEncodedAudioFrame': parameter 1 is not of type 'RTCEncodedAudioFrame'.",
        );
        return;
    };
    ...
    scope
        .get_slot_mut::<RtcEncodedAudioFrameStore>()
        .expect("RTCEncodedAudioFrame state")
        .records
...
pub(crate) fn create<'s>(
    ...
    if crate::webidl::set_platform_prototype(scope, frame, prototype.into()) != Some(true) {
        return Err("cannot create RTCEncodedAudioFrame".to_owned());
    }
    let record = AudioFrameRecord {
        timestamp,
...
fn to_string(
    ...
    if let Some(value) = v8::String::new(scope, "[object RTCEncodedAudioFrame]") {
        result.set(value.into());
    }
}

pub(crate) fn cleanup_realm(scope: &mut v8::PinScope<'_, '_>, realm_id: i32) {
    if let Some(store) = scope.get_slot_mut::<RtcEncodedAudioFrameStore>() {
