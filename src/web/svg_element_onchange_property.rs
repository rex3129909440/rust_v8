...
pub(crate) fn define(
    ...
    crate::webidl::define_accessor(scope, prototype, "onchange", get_value, set_value)
}
...
fn get_value(
    ...
    if let Some(handler) = record.handlers.get("onchange") {
...
fn set_value(
    ...
    if let Some(record) = scope.get_slot_mut::<SvgElementStore>().and_then(|store| {
        ...
        if let Some(handler) = handler {
            record.handlers.insert("onchange".to_owned(), handler);
        } else {
            record.handlers.remove("onchange");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
