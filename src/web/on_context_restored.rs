#[derive(Default)]
pub(crate) struct OnContextRestoredStore {
    handler: Option<v8::Global<v8::Value>>,
}

pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(OnContextRestoredStore::default());
}
...
pub(crate) fn dispatch(
    ...
    if event_type != "contextrestored"
        || !super::window_event_handler_support::is_window(scope, target)
    {
        return;
    }
    let handler = scope
        .get_slot::<OnContextRestoredStore>()
...
fn set_handler(
    ...
    if let Some(store) = scope.get_slot_mut::<OnContextRestoredStore>() {
