...
pub(crate) fn define(
    ...
    crate::webidl::define_accessor(scope, prototype, "onscrollsnapchange", get_value, set_value)
}
...
fn get_value(
    ...
    if let Some(handler) = record.handlers.get("onscrollsnapchange") {
...
fn set_value(
    ...
    if let Some(record) = scope.get_slot_mut::<HtmlElementStore>().and_then(|store| {
        ...
        if let Some(handler) = handler {
            record
                .handlers
                .insert("onscrollsnapchange".to_owned(), handler);
        } else {
            record.handlers.remove("onscrollsnapchange");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
