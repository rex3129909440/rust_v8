...
#[derive(Default)]
pub(crate) struct SvgPointListStore {
    constructor: crate::webidl::RealmConstructor,
    records: HashMap<i32, Vec<super::svg_point::PointValue>>,
}

pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(SvgPointListStore::default());
}

pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let constructor = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "SVGPointList", constructor.into())
}
...
fn ensure_constructor<'s>(
    ...
    scope
        .get_slot_mut::<SvgPointListStore>()
        .ok_or_else(|| "SVGPointList state was not prepared".to_owned())?
        .constructor
...
pub(crate) fn create<'s>(
    scope: &mut v8::PinScope<'s, '_>,
    initial: Vec<super::svg_point::PointValue>,
) -> Result<v8::Local<'s, v8::Object>, String> {
    let constructor = ensure_constructor(scope)?;
    let prototype = crate::webidl::prototype(scope, constructor)?;
    let object = v8::Object::new(scope);
    if crate::webidl::set_platform_prototype(scope, object, prototype.into()) != Some(true) {
        return Err("cannot create SVGPointList".to_owned());
    }
    scope
        .get_slot_mut::<SvgPointListStore>()
        .ok_or_else(|| "SVGPointList state was not prepared".to_owned())?
...
fn illegal_constructor(
    ...
    crate::webidl::throw_type_error(
        scope,
        "Failed to construct 'SVGPointList': Illegal constructor",
    );
}
...
) -> Option<Vec<super::svg_point::PointValue>> {
    scope
        .get_slot::<SvgPointListStore>()?
        .records
...
) -> Option<super::svg_point::PointValue> {
    super::svg_point::value(scope, v8::Local::<v8::Object>::try_from(value).ok()?)
}

fn return_item(
    scope: &mut v8::PinScope<'_, '_>,
    value: super::svg_point::PointValue,
    mut result: v8::ReturnValue<'_>,
) {
    match super::svg_point::create(scope, value) {
...
fn append_item(
    ...
    let Some(items) = scope.get_slot_mut::<SvgPointListStore>().and_then(|store| {
        store
            .records
...
fn clear(
    ...
    if let Some(items) = scope.get_slot_mut::<SvgPointListStore>().and_then(|store| {
        store
            .records
            .get_mut(&arguments.this().get_identity_hash().get())
    }) {
...
fn get_item(
    ...
    let Some(value) = items.get(index).copied() else {
        crate::webidl::throw_type_error(scope, "SVGPointList index is out of bounds");
        return;
    };
    return_item(scope, value, result);
}
