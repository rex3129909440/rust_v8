pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    ...
    let key = crate::webidl::string(scope, "innerWidth")?;
    let global = scope.get_current_context().global(scope);
    if global.define_property(scope, key.into(), &descriptor) == Some(true) {
        Ok(())
    } else {
        Err("cannot define window.innerWidth".to_owned())
    }
}
...
fn get_inner_width(
    ...
    result.set(v8::Number::new(scope, super::window_view_state::inner_width(scope)).into());
}
...
fn set_inner_width(
    ...
    let global = scope.get_current_context().global(scope);
    let Some(key) = v8::String::new(scope, "innerWidth") else {
        return;
    };
