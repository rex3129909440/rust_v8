...
#[derive(Default)]
pub(crate) struct MidiInputMapStore {
    constructor: crate::webidl::RealmConstructor,
    records: HashMap<i32, HashMap<String, v8::Global<v8::Object>>>,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(MidiInputMapStore::default());
}
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let constructor = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "MIDIInputMap", constructor.into())
}
fn ensure_constructor<'s>(
    scope: &mut v8::PinScope<'s, '_>,
) -> Result<v8::Local<'s, v8::Function>, String> {
    if let Some(value) = scope
        .get_slot::<MidiInputMapStore>()
    ...
    scope
        .get_slot_mut::<MidiInputMapStore>()
        .ok_or_else(|| "MIDIInputMap state was not prepared".to_owned())?
        .constructor
...
pub(crate) fn create<'s>(
    ...
    if crate::webidl::set_platform_prototype(scope, object, prototype.into()) != Some(true) {
        return Err("cannot create MIDIInputMap".to_owned());
    }
    ...
    for profile in configured {
        let port = super::midi_input::create(scope, &profile)?;
