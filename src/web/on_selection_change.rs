#[derive(Default)]
pub(crate) struct OnSelectionChangeStore {
    handler: Option<v8::Global<v8::Value>>,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(OnSelectionChangeStore::default());
}
...
pub(crate) fn dispatch(
    ...
    if event_type != "selectionchange"
        || !super::window_event_handler_support::is_window(scope, target)
    {
        return;
    }
    let handler = scope
        .get_slot::<OnSelectionChangeStore>()
...
fn set_handler(
    ...
    if let Some(store) = scope.get_slot_mut::<OnSelectionChangeStore>() {
