...
fn get_value(
    ...
    let reflected = reflected_attribute_name("virtualKeyboardPolicy")
...
fn set_value(
    ...
    if let Some(attribute) = reflected_attribute_name("virtualKeyboardPolicy") {
    ...
    if let Some(record) = scope.get_slot_mut::<HtmlElementStore>().and_then(|store| {
        ...
        record
            .strings
            .insert("virtualKeyboardPolicy".to_owned(), value);
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
    }
}
