#[derive(Default)]
pub(crate) struct OnAbortStore {
    handler: Option<v8::Global<v8::Value>>,
}

pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(OnAbortStore::default());
}
...
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    ...
    let key = crate::webidl::string(scope, "onabort")?;
    let global = scope.get_current_context().global(scope);
    if global.define_property(scope, key.into(), &descriptor) == Some(true) {
        Ok(())
    } else {
        Err("cannot define window.onabort".to_owned())
    }
}
...
fn set_onabort(
    ...
    if let Some(store) = scope.get_slot_mut::<OnAbortStore>() {
