...
#[derive(Default)]
pub(crate) struct CssStartingStyleRuleStore {
    constructor: crate::webidl::RealmConstructor,
    objects: HashSet<i32>,
}

pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(CssStartingStyleRuleStore::default());
}

pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let constructor = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "CSSStartingStyleRule", constructor.into())
}
...
fn ensure_constructor<'s>(
    ...
    if let Some(constructor) = scope
        .get_slot::<CssStartingStyleRuleStore>()
    ...
    scope
        .get_slot_mut::<CssStartingStyleRuleStore>()
        .ok_or_else(|| "CSSStartingStyleRule state was not prepared".to_owned())?
        .constructor
...
pub(crate) fn create<'s>(
    ...
    if crate::webidl::set_platform_prototype(scope, object, prototype.into()) != Some(true) {
        return Err("cannot create CSSStartingStyleRule".to_owned());
    }
