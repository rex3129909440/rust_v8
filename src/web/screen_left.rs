pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    ...
    let key = crate::webidl::string(scope, "screenLeft")?;
    let global = scope.get_current_context().global(scope);
    if global.define_property(scope, key.into(), &descriptor) == Some(true) {
        Ok(())
    } else {
        Err("cannot define window.screenLeft".to_owned())
    }
}
...
fn get_screen_left(
    ...
    let value = super::window_view_state::screen_x(scope);
...
fn set_screen_left(
    ...
    let global = scope.get_current_context().global(scope);
    let Some(key) = v8::String::new(scope, "screenLeft") else {
        return;
    };
