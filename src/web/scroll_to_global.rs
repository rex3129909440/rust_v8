pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    ...
    let key = crate::webidl::string(scope, "scrollTo")?;
    ...
    ) == Some(true)
    {
        ...
        Err("cannot define window.scrollTo".to_owned())
    }
}
...
fn scroll_to(
    ...
    let (x, y) = if let Ok(options) = v8::Local::<v8::Object>::try_from(arguments.get(0)) {
        (
            number_property(scope, options, "left")
                .unwrap_or_else(|| super::window_view_state::scroll_x(scope)),
            number_property(scope, options, "top")
