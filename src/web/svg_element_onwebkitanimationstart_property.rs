...
fn get_value(
    ...
    if let Some(handler) = record.handlers.get("onwebkitanimationstart") {
...
fn set_value(
    ...
    if let Some(record) = scope.get_slot_mut::<SvgElementStore>().and_then(|store| {
        ...
        if let Some(handler) = handler {
            record
                .handlers
                .insert("onwebkitanimationstart".to_owned(), handler);
        } else {
            record.handlers.remove("onwebkitanimationstart");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
