...
#[derive(Default)]
pub(crate) struct ToolbarGlobalStore {
    values: HashMap<i32, v8::Global<v8::Object>>,
}

pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(ToolbarGlobalStore::default());
}
...
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    ...
    scope
        .get_slot_mut::<ToolbarGlobalStore>()
        .ok_or_else(|| "toolbar state was not prepared".to_owned())?
        .values
    ...
    let key = crate::webidl::string(scope, "toolbar")?;
    let global = scope.get_current_context().global(scope);
    if global.define_property(scope, key.into(), &descriptor) == Some(true) {
        Ok(())
    } else {
        Err("cannot define window.toolbar".to_owned())
    }
}
...
fn set_toolbar(
    ...
    let global = scope.get_current_context().global(scope);
    let Some(key) = v8::String::new(scope, "toolbar") else {
        return;
    };
