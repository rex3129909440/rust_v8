pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    ...
    let key = crate::webidl::string(scope, "outerWidth")?;
    let global = scope.get_current_context().global(scope);
    if global.define_property(scope, key.into(), &descriptor) == Some(true) {
        Ok(())
    } else {
        Err("cannot define window.outerWidth".to_owned())
    }
}
...
fn get_outer_width(
    ...
    let value = super::window_view_state::outer_width(scope);
...
fn set_outer_width(
    ...
    let global = scope.get_current_context().global(scope);
    let Some(key) = v8::String::new(scope, "outerWidth") else {
        return;
    };
