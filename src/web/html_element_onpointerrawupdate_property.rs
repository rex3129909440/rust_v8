...
pub(crate) fn define(
    ...
    crate::webidl::define_accessor(scope, prototype, "onpointerrawupdate", get_value, set_value)
}
...
fn get_value(
    ...
    if let Some(handler) = record.handlers.get("onpointerrawupdate") {
...
fn set_value(
    ...
    if let Some(record) = scope.get_slot_mut::<HtmlElementStore>().and_then(|store| {
        ...
        if let Some(handler) = handler {
            record
                .handlers
                .insert("onpointerrawupdate".to_owned(), handler);
        } else {
            record.handlers.remove("onpointerrawupdate");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
