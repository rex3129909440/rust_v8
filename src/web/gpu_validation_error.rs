#[derive(Default)]
pub(crate) struct GpuValidationErrorStore {
    constructor: crate::webidl::RealmConstructor,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(GpuValidationErrorStore::default());
}
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let constructor = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "GPUValidationError", constructor.into())
}
fn ensure_constructor<'s>(
    ...
    if let Some(value) = scope
        .get_slot::<GpuValidationErrorStore>()
    ...
    scope
        ...
        .ok_or_else(|| "GPUValidationError state was not prepared".to_owned())?
        .constructor
...
pub(crate) fn cleanup_realm(scope: &mut v8::PinScope<'_, '_>, realm_id: i32) {
    if let Some(store) = scope.get_slot_mut::<GpuValidationErrorStore>() {
