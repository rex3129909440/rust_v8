...
#[derive(Default)]
pub(crate) struct SvgFeGaussianBlurElementStore {
...
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(SvgFeGaussianBlurElementStore::default());
}
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let c = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "SVGFEGaussianBlurElement", c.into())
}
pub(crate) fn ensure_constructor<'s>(
    scope: &mut v8::PinScope<'s, '_>,
) -> Result<v8::Local<'s, v8::Function>, String> {
    let old = scope
        .get_slot::<SvgFeGaussianBlurElementStore>()
    ...
    super::svg_fe_gaussian_blur_element_in1_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_std_deviation_x_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_std_deviation_y_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_x_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_y_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_width_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_height_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_result_property::define(scope, p)?;
    super::svg_fe_gaussian_blur_element_set_std_deviation::define(scope, p)?;
    ...
    scope
        .get_slot_mut::<SvgFeGaussianBlurElementStore>()
        .ok_or_else(|| "SVGFEGaussianBlurElement state was not prepared".to_owned())?
        .constructor
...
pub(crate) fn create<'s>(
    ...
    let c = ensure_constructor(scope)?;
    let object = super::svg_element::create_with_constructor(scope, c, "feGaussianBlur", owner)?;
    let input = super::svg_animated_string::create(scope, "")?;
    let std_deviation_x = super::svg_animated_number::create(scope, 0.0)?;
    let std_deviation_y = super::svg_animated_number::create(scope, 0.0)?;
...
pub(crate) fn illegal_constructor(
    ...
    crate::webidl::throw_type_error(
        s,
        "Failed to construct 'SVGFEGaussianBlurElement': Illegal constructor",
    )
}
pub(crate) fn record(s: &v8::PinScope<'_, '_>, o: v8::Local<'_, v8::Object>) -> Option<Record> {
    s.get_slot::<SvgFeGaussianBlurElementStore>()?
        .records
