...
#[derive(Clone)]
struct EquirectLayerRecord {
    space: v8::Global<v8::Value>,
    transform: v8::Global<v8::Value>,
    radius: f64,
    central_horizontal_angle: f64,
    upper_vertical_angle: f64,
    lower_vertical_angle: f64,
    on_redraw: Option<v8::Global<v8::Value>>,
}

#[derive(Default)]
pub(crate) struct XrEquirectLayerStore {
    constructor: crate::webidl::RealmConstructor,
    records: HashMap<i32, EquirectLayerRecord>,
}

pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(XrEquirectLayerStore::default());
}

pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let constructor = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "XREquirectLayer", constructor.into())
}
...
fn ensure_constructor<'s>(
    ...
    scope
        .get_slot_mut::<XrEquirectLayerStore>()
        .ok_or_else(|| "XREquirectLayer state missing".to_owned())?
        .constructor
...
pub(crate) fn create<'s>(
    ...
    if crate::webidl::set_platform_prototype(scope, object, prototype.into()) != Some(true) {
        return Err("cannot create XREquirectLayer".to_owned());
    }
    ...
    let radius = options
        .map(|options| super::event::number_property(scope, options, "radius", 0.0))
        .unwrap_or(0.0);
    let central_horizontal_angle = options
        .map(|options| {
            super::event::number_property(
                scope,
                options,
                "centralHorizontalAngle",
                std::f64::consts::TAU,
    ...
    let upper_vertical_angle = options
        .map(|options| {
            super::event::number_property(
                scope,
                options,
    ...
    let lower_vertical_angle = options
        .map(|options| {
            super::event::number_property(
                scope,
                options,
...
fn number_result(
    ...
    arguments: v8::FunctionCallbackArguments<'_>,
    mut result: v8::ReturnValue<'_>,
    select: fn(&EquirectLayerRecord) -> f64,
) {
    if let Some(state) = record(scope, arguments.this()) {
...
fn set_number(
    ...
    arguments: v8::FunctionCallbackArguments<'_>,
    mut result: v8::ReturnValue<'_>,
    assign: fn(&mut EquirectLayerRecord, f64),
) {
    let identity = arguments.this().get_identity_hash().get();
...
fn get_central_horizontal_angle(
    scope: &mut v8::PinScope<'_, '_>,
    arguments: v8::FunctionCallbackArguments<'_>,
    result: v8::ReturnValue<'_>,
) {
    number_result(scope, arguments, result, |state| {
        state.central_horizontal_angle
    })
}

fn set_central_horizontal_angle(
    scope: &mut v8::PinScope<'_, '_>,
    arguments: v8::FunctionCallbackArguments<'_>,
    result: v8::ReturnValue<'_>,
) {
    set_number(scope, arguments, result, |state, value| {
        state.central_horizontal_angle = value
    })
}

fn get_upper_vertical_angle(
    scope: &mut v8::PinScope<'_, '_>,
    arguments: v8::FunctionCallbackArguments<'_>,
    result: v8::ReturnValue<'_>,
) {
    number_result(scope, arguments, result, |state| state.upper_vertical_angle)
}

fn set_upper_vertical_angle(
    scope: &mut v8::PinScope<'_, '_>,
    arguments: v8::FunctionCallbackArguments<'_>,
    result: v8::ReturnValue<'_>,
) {
    set_number(scope, arguments, result, |state, value| {
        state.upper_vertical_angle = value
    })
}

fn get_lower_vertical_angle(
    scope: &mut v8::PinScope<'_, '_>,
    arguments: v8::FunctionCallbackArguments<'_>,
    result: v8::ReturnValue<'_>,
) {
    number_result(scope, arguments, result, |state| state.lower_vertical_angle)
}

fn set_lower_vertical_angle(
    scope: &mut v8::PinScope<'_, '_>,
    arguments: v8::FunctionCallbackArguments<'_>,
    result: v8::ReturnValue<'_>,
) {
    set_number(scope, arguments, result, |state, value| {
        state.lower_vertical_angle = value
    })
}
