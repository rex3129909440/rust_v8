...
fn get_value(
    ...
    if let Some(handler) = record.handlers.get("onwebkitanimationiteration") {
...
fn set_value(
    ...
    if let Some(record) = scope.get_slot_mut::<HtmlElementStore>().and_then(|store| {
        ...
        if let Some(handler) = handler {
            record
                .handlers
                .insert("onwebkitanimationiteration".to_owned(), handler);
        } else {
            record.handlers.remove("onwebkitanimationiteration");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
