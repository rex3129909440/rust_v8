#[derive(Default)]
pub(crate) struct HtmlSpanElementStore {
    constructor: crate::webidl::RealmConstructor,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(HtmlSpanElementStore::default());
}
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let c = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "HTMLSpanElement", c.into())
}
fn ensure_constructor<'s>(
    ...
    scope
        .get_slot_mut::<HtmlSpanElementStore>()
        .ok_or_else(|| "HTMLSpanElement state was not prepared".to_owned())?
        .constructor
...
pub(crate) fn create<'s>(
    ...
    if crate::webidl::set_platform_prototype(scope, o, p.into()) != Some(true) {
        return Err("cannot create HTMLSpanElement".to_owned());
    }
    super::html_element::attach(scope, o, "SPAN");
    Ok(o)
}
