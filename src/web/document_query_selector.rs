...
fn query_selector(
    ...
    if arguments.length() < 1 {
        crate::webidl::throw_type_error(
            scope,
            "Failed to execute 'querySelector' on 'Document': 1 argument required, but only 0 present.",
        );
        return;
    }
    ...
    match super::dom_selector::query_selector_all(scope, arguments.this(), &selector) {
        ...
        Err(_) => {
            super::dom_selector::throw_api_error(scope, "querySelector", "Document", &selector)
        }
    }
}
