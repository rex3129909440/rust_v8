...
fn get_value(
    ...
    if let Some(handler) = record.handlers.get("onsecuritypolicyviolation") {
...
fn set_value(
        ...
        })
    {
        if let Some(handler) = handler {
            record
                .handlers
                .insert("onsecuritypolicyviolation".to_owned(), handler);
        } else {
            record.handlers.remove("onsecuritypolicyviolation");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
