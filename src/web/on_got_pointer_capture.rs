#[derive(Default)]
pub(crate) struct OnGotPointerCaptureStore {
    handler: Option<v8::Global<v8::Value>>,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(OnGotPointerCaptureStore::default());
}
...
pub(crate) fn dispatch(
    ...
    if event_type != "gotpointercapture"
        || !super::window_event_handler_support::is_window(scope, target)
    {
        return;
    }
    let handler = scope
        .get_slot::<OnGotPointerCaptureStore>()
...
fn set_handler(
    ...
    if let Some(store) = scope.get_slot_mut::<OnGotPointerCaptureStore>() {
