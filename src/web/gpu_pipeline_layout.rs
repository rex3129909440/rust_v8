#[derive(Default)]
pub(crate) struct GpuPipelineLayoutStore {
    constructor: crate::webidl::RealmConstructor,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(GpuPipelineLayoutStore::default());
}
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    let constructor = ensure_constructor(scope)?;
    crate::webidl::define_global(scope, "GPUPipelineLayout", constructor.into())
}
fn ensure_constructor<'s>(
    ...
    if let Some(value) = scope
        .get_slot::<GpuPipelineLayoutStore>()
    ...
    scope
        .get_slot_mut::<GpuPipelineLayoutStore>()
        .ok_or_else(|| "GPUPipelineLayout state was not prepared".to_owned())?
        .constructor
...
pub(crate) fn create<'s>(
    ...
    if crate::webidl::set_platform_prototype(scope, object, prototype.into()) != Some(true) {
        return Err("cannot create GPUPipelineLayout".to_owned());
    }
...
pub(crate) fn cleanup_realm(scope: &mut v8::PinScope<'_, '_>, realm_id: i32) {
    if let Some(store) = scope.get_slot_mut::<GpuPipelineLayoutStore>() {
