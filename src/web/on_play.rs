#[derive(Default)]
pub(crate) struct OnPlayStore {
    handler: Option<v8::Global<v8::Value>>,
}
pub(crate) fn prepare(isolate: &mut v8::OwnedIsolate) {
    isolate.set_slot(OnPlayStore::default());
}
pub(crate) fn install(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    super::window_event_handler_support::define(scope, "onplay", get_handler, set_handler)
}
pub(crate) fn dispatch(
    ...
    if event_type != "play" || !super::window_event_handler_support::is_window(scope, target) {
        return;
    }
    let handler = scope
        .get_slot::<OnPlayStore>()
...
fn set_handler(
    ...
    if let Some(store) = scope.get_slot_mut::<OnPlayStore>() {
