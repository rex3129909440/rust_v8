...
pub(crate) fn define(
    ...
    crate::webidl::define_accessor(scope, prototype, "onclick", get_value, set_value)
}
...
fn set_value(
        ...
        })
    {
        if let Some(handler) = handler {
            record.handlers.insert("onclick".to_owned(), handler);
        } else {
            record.handlers.remove("onclick");
        }
    } else {
        crate::webidl::throw_type_error(scope, "Illegal invocation");
        return;
    }
    super::event_target::set_attribute_handler(scope, arguments.this(), "click", present);
}
